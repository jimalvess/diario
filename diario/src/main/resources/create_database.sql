...
--    CRIANDO ABORDAGEM MAIS PROFUNDA pra receber outras midias

-- Dropo a coluna que eu nao vou precisar mais:

ALTER TABLE `diario`.`entrada_diario`
    DROP COLUMN `foto`,
    DROP COLUMN `caminho_imagem`;


-- Crio table soh pras medias:
...
-- Implementando titulo nas entradas:

ALTER TABLE entrada_diario
    add column `titulo` VARCHAR(255) NULL DEFAULT NULL;
commit;