...
import {
    ...
    EyeOutlined,    // Pra visualização de documentos
...
const EditarEntrada = () => {
    ...
    const handleNewFilePreview = async (file) => {
        ...
        setPreviewOpen(true);
        setPreviewModalTitle(file.name || (file.url ? file.url.substring(file.url.lastIndexOf('/') + 1) : 'Visualização'));
    };
    ...
    const handleRemoveExistingMedia = (mediaId) => {
        setExistingMedia(prev => prev.filter(media => media.uid !== mediaId.toString())); // Compara uid string
    ...
    useEffect(() => {
        const fetchEntrada = async () => {
            ...
            try {
                ...
                const loadedMedia = entrada.midias ? entrada.midias.map(media => {
                    ...
                    const mediaUrl = mediaFileName ? `${API_BASE_URL}/api/entradas/arquivo/${mediaFileName}` : null;
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            const response = await axios.put(`${API_BASE_URL}/api/entradas/${id}`, formData, {
                headers: {
            ...
            if (error.response?.status === 413) {
                ...
                message.error('Erro ao atualizar entrada. Verifique o console para mais detalhes.');
            }
        } finally {
    ...
    if (loading) {
        return (
            <Layout style={{ minHeight: '100vh', width: '100vw', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#2c3e50' }}> {/* Fundo escuro simples */}
    ...
    return (
        <Layout style={{ minHeight: '100vh', width: '100vw' }}>
            ...
            >
                ...
                >
                    <Title level={2} style={{ color: 'white', textAlign: 'center', marginBottom: '1.5rem' }}>
                        Edite sua Entrada
                    </Title>
                    ...
                    <Form form={form} layout="vertical" onFinish={onFinish}>
                        <Row gutter={[16, 16]}>
                            <Col span={24}>
                                ...
                                >
                                    <TextArea rows={1} placeholder="Digite o título" style={{ backgroundColor: '#444', color: 'white', borderColor: '#555' }} />
                                </Form.Item>
                            </Col>
                            <Col span={24}>
                                <Form.Item
                                ...
                                >
                                    <TextArea rows={6} placeholder="Como foi seu dia?" style={{ backgroundColor: '#444', color: 'white', borderColor: '#555' }} />
                                </Form.Item>
                            </Col>
                            ...
                            {existingMedia.length > 0 && (
                                <Col span={24}>
                                    <Form.Item label={<span style={{ color: 'white' }}>Mídias Existentes</span>}>
                                            ...
                                            {existingMedia.map(media => {
                                                ...
                                                return (
                                                    <Card
                                                        key={media.uid}
                                                        size="small"
                                                        style={{ width: 120, textAlign: 'center', backgroundColor: 'rgba(0,0,0,0.4)', borderColor: '#555', color: 'white' }}
                                                        ...
                                                        cover={
                                                            isImage ? (
                                                                <Image
                                                                    alt={media.name}
                                                                    src={media.url}
                                                                    style={{ height: 80, objectFit: 'contain', padding: '5px' }}
                                                                    preview={{ src: media.url }} // Habilita a prévia em modal para imagens
                                                                />
                                                            ) : isVideo ? (
                                                                <video controls style={{ height: 80, width: '100%', objectFit: 'contain' }}>
                                                                    <source src={media.url} type={`${media.type}/${media.name.split('.').pop()}`} />
                                                                ...
                                                                <div
                                                                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 80, color: 'white', cursor: 'pointer' }}
                                                                    onClick={() => handleExistingMediaPreview(media.url, media.name, media.type)}
                                                                >
                                                                    <EyeOutlined style={{ fontSize: '30px', color: '#1890ff' }} />
                                                                    <Tag color="blue" style={{ marginTop: '5px' }}>{media.type.replace('_', ' ')}</Tag>
                                                                </div>
                                                            )
                                                        }
                                                        actions={[
                                                            <Tooltip title="Remover Mídia">
                                                                <Button
                                                                    ...
                                                                    onClick={() => handleRemoveExistingMedia(parseInt(media.uid))} // Converte UID de volta para número
                                                                    danger
                                                                />
                                                            </Tooltip>
                                                        ]}
                                                    >
                                                        <Card.Meta title={<span style={{ color: 'white' }}>{media.name}</span>} />
                                                    </Card>
                                                );
                                            })}
                                        </div>
                            ...
                            <Col span={24}>
                                <Form.Item label={<span style={{ color: 'white' }}>Adicionar Novas Mídias (opcional)</span>}>
                                    <Upload {...newUploadProps}>
                                        {newFiles.length + existingMedia.length >= 8 ? null : uploadButton}
                                    </Upload>
                                </Form.Item>
                            </Col>
                        </Row>
            ...
            <Modal open={previewOpen} title={previewModalTitle} footer={null} onCancel={handlePreviewModalCancel} width="80%">
                {previewContentUrl && (
                    previewContentUrl.includes('.pdf') ? (
                        <iframe src={previewContentUrl} style={{ width: '100%', height: '70vh', border: 'none' }} />
                    ) : (
                        // Para outros tipos de documentos, ou visualização de imagem normal
                        <img alt="prévia" style={{ maxWidth: '100%', maxHeight: '70vh', objectFit: 'contain' }} src={previewContentUrl} />
                    )
                )}
            </Modal>

            <Footer style={{ textAlign: 'center', color: '#fff', backgroundColor: '#2c3e50', fontSize: '0.8rem' }}>
